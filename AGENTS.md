...
- CI's second job regenerates everything and fails on any `git diff` — after editing `themes.yml` or any `applications/*/configuration.yml`/`template*.erb`, re-run `ruby generate_themes.rb` and commit the resulting `docs/colors.md` change before pushing.
